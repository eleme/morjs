...
var ScanBleDevice = function (callback, timeout) {
  ...
  wx.offBluetoothDeviceFound() //关闭之前的监听
  console.log('开启蓝牙适配器')
            ...
            wx.getBLEDeviceCharacteristics({
              ...
              success: (res) => {
                ...
                if (typeof ReadId == 'string' && typeof WriteId == 'string') {
                  ...
                  wx.onBLECharacteristicValueChange(GetCharacteristicValue)
...
var GetCharacteristicValue = function (res) {
  let data = changeToHex(res.value).toUpperCase()
  console.log('接收：' + data)
...
var doSendData = function () {
  const value = changeToBuffer(SendDatas[SendIndex])
  wx.writeBLECharacteristicValue({
    deviceId: DeviceId, // 蓝牙设备 id，参考 device 对象
    serviceId: ServiceId, // 蓝牙特征值对应 service 的 uuid
    characteristicId: WriteId, // 蓝牙特征值的 uuid
    value: value, // 蓝牙设备特征值对应的值，16进制字符串，限制在20字节内
    success: (res) => {
